...
void Renderer::DrawSprites(int32_t spriteCount, int32_t vertexOffset)
{
	...
	if (auto emitter = godot::Object::cast_to<godot::EffekseerEmitter>(godotObj)) {
		...
		TransferVertexToImmediate3D(command.GetImmediateMesh(), 
	...
	if (shaderType == RendererShaderType::Unlit)
	{
		...
		for (int32_t i = 0; i < spriteCount; i++)
		{
			...
			for (int32_t j = 0; j < 4; j++)
			{
				...
				mesh->surface_set_color(ConvertColor(v.Col));
	...
	else if (shaderType == RendererShaderType::BackDistortion || shaderType == RendererShaderType::Lit)
	{
		...
		for (int32_t i = 0; i < spriteCount; i++)
		{
			...
			for (int32_t j = 0; j < 4; j++)
			{
				...
				mesh->surface_set_color(ConvertColor(v.Col));
				mesh->surface_set_uv(ConvertUV(v.UV));
				mesh->surface_set_normal(ConvertVector3(Normalize(UnpackVector3DF(v.Normal))));
		...
		if (customData1Count > 0 || customData2Count > 0)
		{
			...
			for (int32_t i = 0; i < spriteCount; i++)
			{
				...
				for (int32_t j = 0; j < 4; j++)
				{
					...
					mesh->surface_set_color(ConvertColor(v.Col));
					mesh->surface_set_uv(ConvertUV(v.UV));
					mesh->surface_set_uv2(ConvertVertexTextureUV(m_vertexTextureOffset++, width));
					mesh->surface_set_normal(ConvertVector3(Normalize(UnpackVector3DF(v.Normal))));
					mesh->surface_set_tangent(ConvertTangent(Normalize(UnpackVector3DF(v.Tangent))));
					mesh->surface_add_vertex(ConvertVector3(v.Pos));
